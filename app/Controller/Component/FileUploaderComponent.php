 ...
 */
 public function upload($formData, $path=null){
  ...
  $nombreNormalizado = str_replace( ' ', '_', strtolower($this->uploadedFile->getFileEntry('name')  )) ;

  $this->uploadedFile->setFileEntry('name', $nombreNormalizado );
  /*
   * */
  ...
  $destination='';
    if (empty($this->filename)) {
     // $this->setFilename( '_' . md5( mt_rand( )) . '.') ;
    }
  // parse out class params to make the final destination string
  if (empty($this->filename)) {

      $fichero =   time().'_'. str_replace( ' ' , '_', strtolower( $this->uploadedFile->getFileEntry('name') ) ) ;

      $destination = $this->destination .  $fichero ;
      $this->setFilename( $fichero );
  } else {
